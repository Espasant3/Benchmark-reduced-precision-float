...
Matrix* _create_Matrix(int rows, int cols) {
    ...
    for(int i = 0; i < rows; i++) {
        matrix->data[i] = calloc(cols, sizeof(__bf16));
    }
    return matrix;
}
...
void standarize_matrix(Matrix* matrix) {
    __bf16 *medias = malloc(matrix->cols * sizeof(__bf16));
    __bf16 *desviaciones = malloc(matrix->cols * sizeof(__bf16));
...
void do_pca(Matrix* matrix) {
    ...
    __bf16* eigenvalues = (__bf16*) malloc(covariance->rows * sizeof(__bf16));
    __bf16* eigenvectors = (__bf16*) malloc(covariance->rows * covariance->cols * sizeof(__bf16));
